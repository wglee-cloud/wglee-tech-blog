...
## 대상 인스턴스
다음 인스턴스를 대상으로 트러블슈팅을 진행하였다. 
wglee-compute-002에 스케줄링 된 인스턴스이다. 
```
...
정상이라면 183.10.10.182 처럼 "network:floatingip" 포트에 대한 mac address를 받아와야 하는데 imcomplete 상태임. 
```
...
이제 공인 아이피의 mac address는 받아온 것을 확인했다. 
```
...
지금 요청이 가지만 가상서버로부터 응답이 안오기 때문에 tap 디바이스에 적용된 방화벽을 의심해 볼 수 있다. 
```
...
방화벽이 compute 노드에 잘 적용 되었는지 확인한다. 
...
즉, sg 에 문제가 있음. 
이전에는 잘 되던 방화벽이었는데 변경된 부분이 있는 것인지 확인해 본다. 
...
5afe811b-4121-4c0e-88b9-50e9802aab1d 방화벽의 룰은 아래와 같다.  tcp 22 허용하는 룰이 없다. 
```
...
default 이름을 가진 방화벽이 project별로 여러개가 있기 때문에 원래 사용하던 ssh 22 허용된 방화벽이 아닌 해당 project에 신규로 생성된 방화벽이 적용된 듯 하다ㅎㅎ
기존에 쓰던 방화벽을 추가로 연결한다. 
```
...
이제 정상적으로 ssh 가 된다. 결국 security group 문제였음 ..^^
